...
def get_biases(sim_i, kd, seed, snap):
...
if os.path.isdir('/mnt/ceph/users/fmaion/projects/bias_priors/biases/redshift_{:.2f}'.format(z)) is False:
    os.mkdir('/mnt/ceph/users//fmaion/projects/bias_priors/biases/redshift_{:.2f}'.format(z))

    if os.path.isdir('/mnt/ceph/users/fmaion/projects/bias_priors/biases/redshift_{:.2f}/SB'.format(z)) is False:
        os.mkdir('/mnt/ceph/users/fmaion/projects/bias_priors/biases/redshift_{:.2f}/SB'.format(z))


seed_dir = "/mnt/ceph/users/camels/Sims/IllustrisTNG_extras/L50n512/SB35/"
kd = 0.3

for sim_i in range(28):
    print("Now measuring simulation {:d}".format(sim_i))
    
    name = "SB35_{:d}".format(sim_i)
    seed = get_seed(seed_dir+name)
    assert(seed==25000+sim_i)

    mstar, SFR, bgal, tr_q, gal_ids = get_biases(sim_i, kd, seed, snap)

    np.save("/mnt/ceph/users/fmaion/projects/bias_priors/biases/redshift_{:.2f}/SB35/".format(z)+name, [{'kd':kd, 'mstar':mstar, 'SFR':SFR, 'bgal':bgal, 'tr_q':tr_q, 'gal_ids':gal_ids}])
