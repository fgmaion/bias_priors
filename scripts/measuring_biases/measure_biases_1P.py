import bacco
import numpy as np
import bacco.probabilistic_bias as pb
import scipy
import os
import sys

bacco.configuration.update({'number_of_threads':16})

project_src = '/mnt/ceph/users/fmaion/projects/bias_priors/src'
sys.path.insert(0, project_src)
os.chdir(project_src)

import utils

def get_biases(basedir, kd, snap):
    
    sigma8 = 0.8159
    ns     = 0.9667
    tau    = 0.0965
    sim_format = 'TNG'
    cm_50 = bacco.Simulation(verbose=False, basedir=basedir, halo_file='groups_%03d/fof_subhalo_tab_%03d'%(snap,snap),\
               dm_file="snapdir_%03d/snap_%03d"%(snap,snap), tau=tau,ns = ns, sigma8=sigma8,\
               numpart=512**3, sim_format=sim_format, use_orphans=False)
    
    cm_50.header['Seed'] = 18
    cm_50.header['InitialPhase'] = 0.0
    cm_50.header['FixedInitialAmplitude'] = False    

    sel = 1e10 * cm_50.sub['MassType'][:,4] > 1e8
    
    sort = np.argsort(cm_50.dm['ids'])
    
    dm_pos = cm_50.dm['pos'][sort]
    dm_vel = cm_50.dm['vel'][sort]
    dm_ids = cm_50.dm['ids'][sort]
    
    pos_tree = scipy.spatial.KDTree(dm_pos)

    # These are the variables that need to be measured on a Lagrangian grid
    D_variables = ("J2", "J2=2")
    D_pbm = pb.ProbabilisticBiasManager(cm_50, variables=D_variables, damping_scale=kd, ngrid=192, filt='sharpk')
    
    D_terms = ("J2", "J22", "J2=2")
    D_model = D_pbm.setup_bias_model(pb.TensorBiasND, terms=D_terms, spatial_order=2)
    
    d_dm, id_dm = pos_tree.query(cm_50.sub['pos'][sel,:]) # This has to be improved, let's explicitly get the mostbound ID and then match it.
    q = utils.dmpos_from_ids(dm_ids[id_dm], 50000/512)
    print("Done! Lagrangian positions have been obtained")
    
    tr_q, tr_value, tr_mask = D_pbm._define_tracers(tracer_q=q/1e3)
    
    b_gal = D_model.bias_per_object(tr_value)
    
    mstar = np.log10(1e10*cm_50.sub['MassType'][sel,4])
    SFR = cm_50.sub['SFR'][sel]
    # mass bins
    
    return mstar, SFR, b_gal, tr_q, np.where(sel)[0]


basedir = "/mnt/ceph/users/camels/Sims/IllustrisTNG/L50n512/1P/"
kd = 0.3

snap = int(sys.argv[1]) # Typical snaps are: 90 for z=0, 74 for z=0.5 and 60 for z=1

sigma8 = 0.8159
ns     = 0.9667
tau    = 0.0965
sim_format = 'TNG'
cm_50 = bacco.Simulation(verbose=False, basedir=basedir+"1P_p1_0", halo_file='groups_%03d/fof_subhalo_tab_%03d'%(snap,snap),\
           dm_file="snapdir_%03d/snap_%03d"%(snap,snap), tau=tau,ns = ns, sigma8=sigma8,\
           numpart=512**3, sim_format=sim_format, use_orphans=False)

z = 1. / cm_50.Cosmology.expfactor - 1.

if os.path.isdir('/mnt/home/fmaion/storage/projects/bias_priors/biases/redshift_{:.2f}'.format(z)) is False:
    os.mkdir('/mnt/home/fmaion/storage/projects/bias_priors/biases/redshift_{:.2f}'.format(z))

    if os.path.isdir('/mnt/home/fmaion/storage/projects/bias_priors/biases/redshift_{:.2f}/1P'.format(z)) is False:
        os.mkdir('/mnt/home/fmaion/storage/projects/bias_priors/biases/redshift_{:.2f}/1P'.format(z))

for par_i in range(1,36):
    print("Starting with parameter {:d}".format(par_i))
    if par_i==15:
        names = ["1P_p{:d}_0".format(par_i),
                "1P_p{:d}_1".format(par_i),
                "1P_p{:d}_2".format(par_i),
                "1P_p{:d}_3".format(par_i),
                "1P_p{:d}_4".format(par_i)] 
    elif par_i==1:
        names = ["1P_p{:d}_n2".format(par_i),
                "1P_p{:d}_n1".format(par_i),
                "1P_p{:d}_0".format(par_i),
                "1P_p{:d}_1".format(par_i),
                "1P_p{:d}_2".format(par_i)]
    elif par_i==29:
        names = ["1P_p{:d}_n1".format(par_i),
                "1P_p{:d}_1".format(par_i),
                "1P_p{:d}_2".format(par_i),
                "1P_p{:d}_3".format(par_i)]
    elif par_i==30 or par_i==32:
        names = ["1P_p{:d}_n1".format(par_i),
                "1P_p{:d}_n2".format(par_i),
                "1P_p{:d}_n3".format(par_i),
                "1P_p{:d}_n4".format(par_i)] 
    else:
        names = ["1P_p{:d}_n2".format(par_i),
                "1P_p{:d}_n1".format(par_i),
                "1P_p{:d}_1".format(par_i),
                "1P_p{:d}_2".format(par_i)]
    
    for name_i in range(len(names)):
        print("Now going through simulation " + names[name_i])
        mstar, SFR, bgal, tr_q, gal_ids = get_biases(basedir+names[name_i], kd, snap)

        np.save("/mnt/ceph/users/fmaion/projects/bias_priors/biases/redshift_{:.2f}/1P/".format(z)+names[name_i], [{'kd':kd, 'mstar':mstar, 'SFR':SFR, 'bgal':bgal, 'tr_q':tr_q, 'gal_ids':gal_ids}])

